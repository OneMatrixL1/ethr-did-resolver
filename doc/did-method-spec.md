...
The `did:vbsn` method provides support for Decentralized Identifiers (DIDs) on the (VBSN blockchain networks)[https://vbsn.vn].

The `did:vbsn` method is largely based on and similar to the `did:ethr` method. The primary differences are the method
name (`vbsn` instead of `ethr`) and the target network, which is the VBSN blockchain identified by its own chain ID.

The DID method specification conforms to the requirements specified in
...
By default, each identifier is controlled by itself, or rather by its corresponding VBSN address. Each identifier can
only be controlled by a single address at any given time. The controller can replace themselves with any other address,
including contracts to allow more advanced models such as multi-signature control.

## Target System

The target system is the VBSN network where the ERC1056 contract is deployed. The VBSN network is identified by its
chain ID: `0xd2f0`.
...
- Uses built-in account abstraction
...
- Supports decoupling chain interaction from the underlying identifier
...
To enable JSON-LD processing, the `@context` used when constructing DID documents for `did:vbsn` should be:
...
The namestring that shall identify this DID method is: `vbsn`

A DID that uses this method MUST begin with the following prefix: `did:vbsn`. Per the DID specification, this string
...
or the corresponding HEX-encoded address on the target network, prefixed with `0x`.

    vbsn-did = "did:vbsn:" vbsn-specific-identifier
    vbsn-specific-identifier = [ vbsn-network ":" ] address / public-key-hex
    vbsn-network = network-chain-id ( Ethereum-based mainnet is 0xd2f0)
    network-chain-id = "0x" *HEXDIG
    address = "0x" 40*HEXDIG
    public-key-hex = "0x" 66*HEXDIG

The `address` or `public-key-hex` are case-insensitive, however, the corresponding `blockchainAccountId`
...
If no vbsn-network is specified, the default VBSN chain ID is assumed to be 54000 or 0xd2f0 in hexadecimal.

  did:vbsn:0xd2f0:0xb9c5714089478a327f09197987f16f9e5d936e8a
  did:vbsn:0xb9c5714089478a327f09197987f16f9e5d936e8a
...
- all Read, Update, and Delete operations MUST be made using the corresponding `blockchainAccountId` and MUST originate
  from the correct controller account (ERC1056 `owner`).

## Relationship to ERC1056

The subject of a `did:vbsn` is mapped to an `identity` address in the ERC1056 contract. When dealing with public key
identifiers, the address corresponding to that public key is used to represent the controller.

The controller address of a `did:vbsn` is mapped to the `owner` of an `identity` in the ERC1056.
...
Rather, this address is a concept specific to ERC1056 and defines the address that is allowed to perform Update and
...
In order to create a `vbsn` DID, a key pair needs to be generated. At this point, no interaction with the VBSN network
is required. The registration is implicit as it is impossible to brute force an address, i.e., guessing the private key
for a given public key on the Koblitz Curve (secp256k1). The holder of the private key is the entity identified by the
DID.

The default DID document for a `did:vbsn:<address>`, e.g.
`did:vbsn:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74` with no transactions to the ERC1056 registry looks like this:
...
```json
{
  ...
  "id": "did:vbsn:0xb9c5714089478a327f09197987f16f9e5d936e8a",
  "verificationMethod": [
    {
      "id": "did:vbsn:0xb9c5714089478a327f09197987f16f9e5d936e8a#controller",
      "type": "EcdsaSecp256k1RecoveryMethod2020",
      "controller": "did:vbsn:0xb9c5714089478a327f09197987f16f9e5d936e8a",
      "blockchainAccountId": "eip155:<VBSN_CHAIN_ID>:0xb9c5714089478a327f09197987f16f9e5d936e8a"
    }
  ],
  "authentication": [
    "did:vbsn:0xb9c5714089478a327f09197987f16f9e5d936e8a#controller"
  ],
  "assertionMethod": [
    "did:vbsn:0xb9c5714089478a327f09197987f16f9e5d936e8a#controller"
  ]
...
The minimal DID Document for a `did:vbsn:<public key>` where there are no corresponding TXs to the ERC1056 registry
looks like this:
...
```json
{
  ...
  "id": "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
  "verificationMethod": [
    {
      "id": "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798#controller",
      "type": "EcdsaSecp256k1RecoveryMethod2020",
      "controller": "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
      "blockchainAccountId": "eip155:<VBSN_CHAIN_ID>:0xb9c5714089478a327f09197987f16f9e5d936e8a"
    },
    {
      "id": "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798#controllerKey",
  ...
  "authentication": [
    "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798#controller",
    "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798#controllerKey"
  ],
  "assertionMethod": [
    "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798#controller",
    "did:vbsn:0x0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798#controllerKey"
...
Any value from the registry that returns an address will be added to the `verificationMethod` array of the DID document
with type `EcdsaSecp256k1RecoveryMethod2020` and a `blockchainAccountId` attribute containing the address
...
If a change has ever been made for the address of an identifier the block number is stored in the `changed` mapping of
the contract.
...
1. Call `changed(address identity)` on the ERC1056 contract to get the latest block where a change occurred.
...
When the controller address of a `did:vbsn` is changed, a `DIDOwnerChanged` event is emitted.
...
Delegate keys are addresses that can either be general signing keys or optionally also perform authentication.
...
- `veriKey` which adds a `EcdsaSecp256k1RecoveryMethod2020` to the `verificationMethod` section of the DID document with
  the `blockchainAccountId` of the delegate, and adds a reference to it in the `assertionMethod` section.
...
Note, the `delegateType` is a `bytes32` type for gas efficiency reasons and not a `string`. This restricts us to 32
bytes, which is why we use the shorthand versions above.
...
```json
{
  "id": "did:vbsn:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74#delegate-1",
  "type": "EcdsaSecp256k1RecoveryMethod2020",
  "controller": "did:vbsn:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74",
  "blockchainAccountId": "eip155:<VBSN_CHAIN_ID>:0x12345678c498d9e26865f34fcaa57dbb935b0d74"
}
```

##### Attributes (`DIDAttributeChanged`)

Non-native keys, service endpoints etc. can be added using attributes. Attributes only exist on the blockchain as
...
Note, the name is a `bytes32` type for gas efficiency reasons and not a `string`. This restricts us to 32 bytes, which
is why we use the shorthand attribute versions explained below.
...
```json
{
  "id": "did:vbsn:0xf3beac30c498d9e26865f34fcaa57dbb935b0d74#service-1",
  "type": "HubService",
...
These functions will trigger the respective EVM events which are used to build the DID Document for a given account as
described
...
the respective events which are used to build the DID Document for a given identifier as described
in [Enumerating Contract Events to build the DID Document](#Enumerating-Contract-Events-to-build-the-DID-Document).

### Delete (Revoke)

The `owner` property of the identifier MUST be set to `0x0`. Although, `0x0` is a valid address, this will indicate the
account has no owner which is a common approach for invalidation, e.g., tokens. To detect if the `owner` is the `null`
address, one MUST get the logs of the last change to the account and inspect if the `owner` was set to the null address
(`0x0000000000000000000000000000000000000000`). It is impossible to make any other changes to the DID document after
such a change, therefore all preexisting keys and services MUST be considered revoked.
...
Example: `did:vbsn:0x26bf14321004e770e7a8b080b7a526d8eed8b388?versionId=12090175`
